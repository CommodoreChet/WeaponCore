    ...
    public partial class Projectiles
    {
        internal void InitialHitCheck(Projectile p, bool lineCheck)
        {
            ...
            for (int i = 0; i < collectionCount; i++)
            {
                ...
                {
                    if (shieldInfo == null)
                        shieldInfo = Session.I.SApi.MatchEntToShieldFastExt(ent, true);
                            ...
                            if (Vector3D.Transform(!shrapnelSpawn ? info.Origin : coreEntity.PositionComp.WorldMatrixRef.Translation, shieldInfo.Value.Item3.Item1).LengthSquared() > 1)
                            {
                                ...
                                if (shieldIntersect && !info.ShieldBypassed)
                                {
                                    ...
                                    if (shieldInfo.Value.Item2.Item2)
                                    {
                                        ...
                                        var faceInfo = Session.I.SApi.GetFaceInfo(shieldInfo.Value.Item1, hitPos);
        ...
        internal static void SendClientHit(Projectile p, bool hit)
        {
            ...
            Session.I.SendFixedGunHitEvent(hit, comp.CoreEntity, info.Hit.Entity, intersectOrigin, vel, info.OriginUp, info.MuzzleId, w.System.WeaponIdHash, aConst.AmmoIdxPos, (float)(isBeam ? info.MaxTrajectory : distToTarget));
                ...
                try
                {
                    ...
                    try
                    {
                        if (finalCount > 0)
                        {
                            ...
                            info.Hit = new Hit { Entity = hitEntity.Entity, EventType = hitEntity.EventType, LastHit = visualHitPos ?? Vector3D.Zero, SurfaceHit = visualHitPos ?? Vector3D.Zero, HitVelocity = lastHitVel ?? Vector3D.Zero, HitTick = Session.I.Tick };
            ...
            for (int i = 0; i < DeferedVoxels.Count; i++)
            {
                ...
                if (branch == VoxelIntersectBranch.DeferFullCheck)
                {
                    ...
                    else
                    {
                        using (voxel.Pin())
                        {
                            if (!voxel.GetIntersectionWithLine(ref p.Beam, out voxelHit, true, IntersectionFlags.DIRECT_TRIANGLES) && VoxelIntersect.PointInsideVoxel(voxel, s.TmpStorage, p.Beam.From))
